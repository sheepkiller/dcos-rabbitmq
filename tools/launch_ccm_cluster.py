...
class CCMLauncher(object):
    ...
    def _start(self, config):
        ...
        if is_17_cluster:
        ...
        elif config.cf_rabbitmq.startswith('ee.'):
            ...
            if not mode:
                logger.warning("No rabbitmqs known for enterprise & "
                        "default security (none). Cowardly bringing "
                        "up a permissive cluster")
                mode = 'permissive'
            rabbitmq_url = 'https://{}/{}/{}/cloudformation/{}'.format(
                hostrepo, config.ccm_channel, mode, config.cf_rabbitmq)
        else:
            hostrepo = 's3-us-west-2.amazonaws.com/downloads.dcos.io/dcos'
        # non-ee mode
        if not rabbitmq_url:
            rabbitmq_url = 'https://{}/{}/cloudformation/{}'.format(
                hostrepo, config.ccm_channel, config.cf_rabbitmq)
        # external override from DCOS_RABBITMQ_URL
        if config.rabbitmq_url:
            rabbitmq_url = config.rabbitmq_url
            logger.info("Accepting externally provided rabbitmq_url from environment.")
  ...
  rabbitmq_url={}'''.format(
      ...
      rabbitmq_url))
        ...
        if config.postlaunch_steps != 'none':
            is_enterprise = config.cf_rabbitmq.startswith('ee.')
...
class StartConfig(object):
    ...
    def __init__(
            ...
            cf_rabbitmq = 'ee.single-master.cloudformation.json',
        ...
        self.cf_rabbitmq = os.environ.get('CCM_RABBITMQ', cf_rabbitmq)
        ...
        if not self.security_mode in ('strict', 'permissive', None):
            raise Exception("Unknown value for SECURITY: %s" %
                    self.security_mode)
        self.rabbitmq_url = os.environ.get('DCOS_RABBITMQ_URL', None)
        if not description:
