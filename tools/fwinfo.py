...
if __name__ == "__main__":
    ...
    class tests(unittest.TestCase):
        ...
        def test_add_with_root(self):
            add_framework('rabbitmq', repo_root=abs_repo_root)
            self.assertEqual(get_framework_names(), ['rabbitmq'])

        def test_add_without_root(self):
            init_repo_root(abs_repo_root)
            self.assertFalse(have_framework('rabbitmq'))
            add_framework('rabbitmq')
            self.assertTrue(have_framework('rabbitmq'))
            self.assertEqual(get_framework_names(), ['rabbitmq'])

        def test_add_dupe_framework(self):
            init_repo_root(abs_repo_root)
            add_framework('rabbitmq')
            self.assertRaises(Exception, add_framework, 'rabbitmq')
