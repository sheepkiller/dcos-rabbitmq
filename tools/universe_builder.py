...
class UniversePackageBuilder(object):
    ...
    def _get_rabbitmq_mapping_for_content(self, orig_content):
        '''Returns a rabbitmq mapping (dict) for the following cases:
        ...
        - Custom environment params like 'RABBITMQ_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default rabbitmq values (may be overridden via eg RABBITMQ_PACKAGE_VERSION envvars):
        rabbitmq_mapping = {
        ...
        # look for any 'sha256:filename' rabbitmq params, and get shas for those.
        # this avoids calculating shas unless they're requested by the rabbitmq.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            rabbitmq_mapping['sha256:{}'.format(shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom RABBITMQ_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('RABBITMQ_'):
                # 'RABBITMQ_SOME_KEY' => 'some-key'
                rabbitmq_mapping[env_key[len('RABBITMQ_'):].lower().replace('_', '-')] = env_val

        return rabbitmq_mapping
...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <package-version> <rabbitmq-package-dir> <artifact-base-path> [artifact files ...]'.format(argv[0]))
    logger.info('  Example: $ {} kafka 1.2.3-4.5.6 /path/to/rabbitmq/jsons/ https://example.com/path/to/kafka-artifacts /path/to/artifact1.zip /path/to/artifact2.zip /path/to/artifact3.zip'.format(argv[0]))
    logger.info('In addition, environment variables named \'RABBITMQ_SOME_PARAMETER\' will be inserted against the provided package rabbitmq (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # local path where the package rabbitmq is located:
